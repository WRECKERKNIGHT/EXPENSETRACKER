...
import { Category, Expense, TransactionType, PaymentMethod } from '../types';
...
import { X, Sparkles, Loader2, Check, ArrowDownCircle, ArrowUpCircle, ScanLine, Smartphone, UploadCloud, Image as ImageIcon, Clipboard, FileText, AlertCircle, Tag } from 'lucide-react';
...
const AddExpenseModal: React.FC<AddExpenseModalProps> = ({ isOpen, onClose, onAdd, initialText, initialMode = 'auto' }) => {
  ...
  // Edit mode for itemized labeling
  ...
  const confirmDetectedTransactions = () => {
    ...
    const validExpenses = detectedTransactions.map(t => ({
      ...
      paymentMethod: (t.paymentMethod as PaymentMethod) || 'online'
    }));
  ...
  // Update a single detected transaction's label/category
  const updateDetectedTransaction = (index: number, field: string, value: string) => {
    if (!detectedTransactions) return;
    const updated = [...detectedTransactions];
    updated[index] = { ...updated[index], [field]: value };
    setDetectedTransactions(updated);
  };

  // Remove a detected transaction
  const removeDetectedTransaction = (index: number) => {
    if (!detectedTransactions) return;
    const updated = detectedTransactions.filter((_, i) => i !== index);
    setDetectedTransactions(updated.length > 0 ? updated : null);
  };
             ...
             <div className="space-y-4 animate-fade-in">
                  <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-xl">
                      <div className="flex items-center gap-2 text-emerald-400">
                          <Check size={18} />
                          <span className="text-sm font-bold">Success! Review & Label Items</span>
                      </div>
                  ...
                  <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                      {detectedTransactions.map((tx, idx) => (
                          <div key={idx} className="bg-zinc-900/80 border border-zinc-800 p-4 rounded-xl">
                              {editingIndex === idx ? (
                                  // Edit mode for itemized labeling
                                  <div className="space-y-3">
                                      <div className="flex items-center justify-between">
                                          <span className="text-xs font-bold text-orange-400 uppercase tracking-wider">Edit Item</span>
                                          <button onClick={() => setEditingIndex(null)} className="text-zinc-500 hover:text-white text-xs">Done</button>
                                      </div>
                                      <input
                                          type="text"
                                          value={tx.description || ''}
                                          onChange={(e) => updateDetectedTransaction(idx, 'description', e.target.value)}
                                          placeholder="What was purchased?"
                                          className="w-full bg-black/30 border border-zinc-700 rounded-xl px-4 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50"
                                      />
                                      <div className="grid grid-cols-2 gap-2">
                                          <select
                                              value={(tx.category as string) || 'Other'}
                                              onChange={(e) => updateDetectedTransaction(idx, 'category', e.target.value)}
                                              className="bg-black/30 border border-zinc-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50 appearance-none"
                                          >
                                          ...
                                          <select
                                              value={(tx.paymentMethod as string) || 'online'}
                                              onChange={(e) => updateDetectedTransaction(idx, 'paymentMethod', e.target.value)}
                                              className="bg-black/30 border border-zinc-700 rounded-xl px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50 appearance-none"
                                          >
                                  ...
                                  <div className="flex items-center justify-between gap-4">
                                          ...
                                          <div>
                                              ...
                                              <div className="flex gap-2 text-xs text-zinc-500 mt-1">
                                                  ...
                                                  {tx.paymentMethod && tx.paymentMethod !== 'other' && (
                                                      <span className="bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded border border-blue-500/20">{tx.paymentMethod}</span>
                                                  )}
                                              </div>
                                          </div>
                                      </div>
                                      <div className="flex items-center gap-2">
                                          ...
                                          <button
                                              ...
                                              className="p-1.5 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-zinc-400 hover:text-orange-400 transition-all"
                                              title="Edit label"
                                          >
                                              <Tag size={14} />
                                          </button>
                                          <button
                                              onClick={() => removeDetectedTransaction(idx)}
                                              className="p-1.5 bg-zinc-800 hover:bg-zinc-700 rounded-lg text-zinc-400 hover:text-red-400 transition-all"
                                              title="Remove"
                                          >
                                              <X size={14} />
                                          </button>
            ...
            <>
                ...
                {activeTab === 'manual' && (
                    <form onSubmit={handleManualSubmit} className="space-y-6">
                        ...
                        <div>
                            <label className="block text-sm text-zinc-400 mb-2 ml-1 font-medium tracking-wide">PAYMENT METHOD</label>

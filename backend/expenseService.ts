...
export const createExpense = async (
  userId: string,
  { amount, category, type, date, description, paymentMethod }: Omit<Expense, 'id' | 'userId' | 'createdAt'>
): Promise<Expense> => {
  const id = uuidv4();
  const now = Date.now();
  ...
  await runQuery(
    ...
    [id, userId, amount, category, type, date, description, method, now, now]
  );

  return { id, userId, amount, category, type, date, description, paymentMethod: method, createdAt: now };
};

export const getExpenses = async (userId: string): Promise<Expense[]> => {
  return allQuery(
    'SELECT id, userId, amount, category, type, date, description, paymentMethod, createdAt FROM expenses WHERE userId = ? ORDER BY date DESC',
    [userId]
  );
};
...
export const getExpenseById = async (id: string, userId: string): Promise<Expense | null> => {
  return getQuery(
    'SELECT id, userId, amount, category, type, date, description, paymentMethod, createdAt FROM expenses WHERE id = ? AND userId = ?',
    [id, userId]
  );
};
